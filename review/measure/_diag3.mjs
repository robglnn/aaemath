/** scratch diagnostic 3 — deleted before handoff */
...
await itemBank.ensure(graphSource.knowledgePoints ? graphSource.knowledgePoints.map((k) => k.id) : []);

const ability = Number(process.argv[2] ?? 1.4);
...
const scheduler = new Scheduler(mastery, { clock, seed: 7, sessionMinutes: 25, bank: itemBank });
...
for (let n = 0; n < 12; n += 1) {
  ...
  for (let g = 0; g < 600; g += 1) {
    ...
    session.submit(req, {
      ...
      latencyMs,
      itemId: drawn?.item?.id ?? `${req.kpId}#${req.seq}`,
      family: drawn?.family ?? undefined,
      hinted: req.hinted,
    });
    t.ms += 4000;
  }
  ...
  console.log(
    ...
    "kinds=", JSON.stringify(kinds), "kps=", kps.size, [...kps].join(","),
    "close=", session.closeReason, "win=", session.closingWin,
    "| L1%=", sum.level1Percent, "prov=", sum.provisional, "mast=", sum.mastered, "certs=", mastery.stats.certifications
  );
}
console.log("unservable requests:", unservable, "stats:", JSON.stringify(mastery.stats));
