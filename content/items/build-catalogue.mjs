...
/**
 * The shipped module. Data is inlined rather than imported as JSON on purpose: a plain ESM
 * module loads identically under Node (the offline simulations and review/measure/P17.mjs) and
 * under Vite (the game), with no import-attribute dialect in between. The per-knowledge-point
 * files under `bank/` stay as the readable, reviewable artefact; `review/measure/P17.mjs`
 * asserts the two are the same data, so "inlined" can never drift into "different".
 */
const ids = kg.nodes.map((n) => n.id);
const files = ids.map((id) => JSON.parse(fs.readFileSync(path.join(bankDir, `${id}.json`), "utf8")));
const barrel = `/**
 * index.mjs — the shipped item bank, as one module.
 *
 * GENERATED by content/items/build-catalogue.mjs — do not hand-edit. Change a generator family
 * or a string and re-run the builder. The same data lives, readably, one file per knowledge
 * point, under content/items/bank/; review/measure/P17.mjs asserts they agree.
 */
export const BANK = ${JSON.stringify(files)};
export const BANK_INDEX = ${JSON.stringify(index)};
