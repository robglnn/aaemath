...
for(const kp of GRAPH.ids){ if(GRAPH.difficulty(kp)<3) continue;
...
for(let r=0;r<N;r++){
 ...
 const easy=(kp)=>GRAPH.difficulty(kp)<=2;
 for(let s=0;s<26;s++){
 ...
 const hard=via.filter(id=>GRAPH.difficulty(id)>=3);
...
console.log("band>=3 per-kp offers/passes:",Object.entries(PERKP).filter(([k])=>GRAPH.difficulty(k)>=3).map(([k,v])=>k+" "+v.off+"/"+v.pass).join(", "));
