...
export class Animator {
  ...
  _commit(pose, dt) {
    ...
    for (let i = 0; i < 2; i++) {
      const tag = i === 0 ? "L" : "R";
      const sign = i === 0 ? -1 : 1;
      // Signs are correct for a +Z-forward model (Avatar's `prism` authors the front toward +Z).
      // Rotating a limb hanging at (0,-1,0) about X by θ sends its tip to z' = -sin θ, so a POSITIVE
      // knee swings the heel to -Z (backwards, correct for a knee) and a NEGATIVE elbow brings the
      // forearm to +Z (forwards, correct for an elbow). These were once negated wholesale on the
      // theory that the rig was authored against the body's facing; it is not, and the flip bent
      // every hinge the wrong way. Leave them.
